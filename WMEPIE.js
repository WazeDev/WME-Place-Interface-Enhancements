...
// @require      https://cdn.jsdelivr.net/npm/@turf/turf@7.3.2/turf.min.js
...
// @require      https://raw.githubusercontent.com/karlsosha/WME-Utils/refs/heads/useSDK/SDKGoogleLinkEnhancer.js
...
function pie(tries = 1) {
    ...
    **/
    const GeoJSONOrthogonalizeGeometry = function (geometry, threshold = 12) {
    ...
    function init2() {
        ...
        //     "Creates a residential Place point",
        //     "wmepie",
        ...
        try {
            ...
            const rppShortcut = {
                ...
                description: "Creates a residential Place point",
                callback: () => {
                    startPlacementMode(resCategory, true);
                },
    ...
    function PlaceMenuShortcut(itemNum) {
        ...
        if (selected?.objectType === "venue") {
            ...
            if (selectedVenue.categories.indexOf(cat) === -1) {
    ...
    async function createPlace(geometry, category, _isPoint) {
        ...
        const newPlace = sdk.DataModel.Venues.addVenue({ category: category, geometry });

        // NewPlace.attributes.categories.push(category);
        if (category === resCategory) {
            const np = { isEntry: true, isPrimary: true, point: geometry };
            sdk.DataModel.Venues.replaceNavigationPoints( { navigationPoints: [np], venueId: newPlace.toString() } );
        ...
        // W.model.actionManager.add(new AddPlace(NewPlace));
        const sdkSeg = sdk.DataModel.Segments.getById({ segmentId: closestSeg.segment.id });
        if (sdkSeg) {
            ...
            if (settings.UseCityFromClosestSeg) {
                ...
                if (settings.UseAltCity && city.name === "") {
                    ...
                    if (sdkSeg.alternateStreetIds.length > 0) {
                        ...
                        for (let j = 0; j < sdkSeg.alternateStreetIds.length; j++) {
                            const alt = sdkSeg.alternateStreetIds[j];
                            const altAddr = sdk.DataModel.Streets.getById({ streetId: alt });
                            const altCity = sdk.DataModel.Cities.getById({ cityId: altAddr.cityId });
                            if (altCity.name !== null) {
                                 streetId = alt;
                                 break;
                            }
                        }
            ...
            const selection = { objectType: 'venue', ids: [ newPlace.toString() ] };
            sdk.Editing.setSelection( { selection } );
    ...
    async function editRPPAddress(rppTries = 1) {
        const edview = $(".address-edit-view");
        if (edview.length > 0 && edview[0].checkVisibility()) {
            $(".full-address").trigger("click");
            await new Promise((r) => setTimeout(r, 100));
            $("input", $(".house-number")[0].shadowRoot).focus();
        } else if (rppTries < 1000) {
            console.log("PIE: address-edit-view not found");
            setTimeout(() => {
        ...
        if (selected !== null) {
            if (selected.objectType === "venue") {
                ...
                if (venue?.categories.includes("PARKING_LOT")) {
                    ...
                    waitForElementLoaded('#venue-edit-general > .categories-control').then(() => {
                        $("#venue-edit-general > .categories-control > .label-with-tooltip").after( $ParkingSpotEstimatorButton );
                    });
    ...
    function disablePLSpotEstimatorDrawMode() {
        ...
        sdk.Map.removeAllFeatures({ layerName: layerConfig.PIEPLSpotEstimatorLayer.layerName });
        sdk.Map.removeLayer({ layerName: layerConfig.PIEPLSpotEstimatorLayer.layerName });
    ...
    function ShowClearDescription() {
        setTimeout(() => {
            ...
            $(".clearButton").on("click", () => {
                const selected = sdk.Editing.getSelection();
                if (selected?.objectType === "venue") {
                    sdk.DataModel.Venues.updateVenue({ venueId: selected.ids[0], description: "" });
                }
            });
        }, 0);
    }
        ...
        setTranslations({
            en: {
                prefs: {
                    ...
                    ShowPLANameTitle: "",
                    Item: "Item",
                    PlaceMenuCustomization: "Create Place Custom shortcuts",
